...
This script processes regex patterns from the local regex_golf_puzzles file,
makes separate GPT queries for each pattern, and outputs training data in the format:
...
class RegexGolfDataGenerator:
    ...
    def __init__(self, 
                 ...
                 output_file: str = "./generated_data/regex_golf_dataset_puzzle.json"):
        """
    ...
    def load_regex_dataset(self) -> List[Dict[str, Any]]:
        """Load all regex patterns from the local regex_golf_puzzles file"""
        logger.info("Loading regex patterns from regex_golf_puzzles file...")
        
        # Path to the regex_golf_puzzles file
        puzzles_file = Path(__file__).parent / "regex_golf_puzzles"
            ...
            with open(puzzles_file, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    if line and not line.startswith('#'):  # Skip empty lines and comments
                        # Remove the leading '/' and trailing '/' with flags
                        if line.startswith('/') and line.rfind('/') > 0:
                            last_slash = line.rfind('/')
                            regex_pattern = line[1:last_slash]
                        ...
                        regex_data.append({
                            "regex": regex_pattern,
                            "description": f"Regex golf puzzle #{line_num}" + (f" (flags: {flags})" if flags else ""),
                            "examples": []  # No pre-existing examples for puzzle format
                        })
                        
        except FileNotFoundError:
            logger.error(f"Could not find regex_golf_puzzles file at {puzzles_file}")
            return []
            
        logger.info(f"Loaded {len(regex_data)} regex patterns from puzzles file")
        return regex_data
...
def main():
    ...
    logger.info(f"⏱️ Total generation time: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
